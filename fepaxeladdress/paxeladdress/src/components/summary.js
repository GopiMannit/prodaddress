...
const Summary = () => {
  ...
  const loadUserData = () => {
    axios
      .get(`https://addressmanager-api.mannit.co/address?companyId=${companyId}`)
      .then((response) => {
...
const drawBox = (doc, x, y, tcontent) => {
  //uncomment the below line to draw box
...
const drawBoxes = (doc) => {
  ...
  for (let i = 0; i < BOXES_PER_PAGE; i++) {
    if (i % 24 === 0 && i !== 0) {
      ...
      col = 0;
      const x = (PAGE_WIDTH - (BOX_WIDTH * BOXES_PER_ROW + BOX_MARGIN * (BOXES_PER_ROW - 1))) / 2 + row * (BOX_WIDTH + BOX_MARGIN);
      const y = TOP_MARGIN + col * (BOX_HEIGHT + 0);
      drawBox(doc, x, y, content[i]);
      row++;
    } else {
      const x = (PAGE_WIDTH - (BOX_WIDTH * BOXES_PER_ROW + BOX_MARGIN * (BOXES_PER_ROW - 1))) / 2 + row * (BOX_WIDTH + BOX_MARGIN * 1.5);
      const y = TOP_MARGIN + col * (BOX_HEIGHT + 0);
      drawBox(doc, x, y, content[i]);
      row++;
      if (row === BOXES_PER_ROW) {
...
// const companyId = sessionStorage.getItem('username');

const handleExportRows = (rows) => {
  rows.filter((row) => row.original.companyId === companyId).map((row) => content.push([row.original.pre + ". " + row.original.fullname, row.original.address1, row.original.address2, row.original.city, row.original.region, row.original.zipcode, row.original.country, "(Ph :) " + row.original.phone + "  (Ph :) " + row.original.phone1]))
  drawBoxes(doc);
  doc.save("export.pdf");
  // console.log(content);
  setcontent([]);
  ...
  return (
    <div className={companyId==="PAXEL" ? 'bg-[#ffc107] h-screen' : 'bg-[#00203f]'}>
